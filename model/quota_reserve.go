...
// Redis 明确未命中时降级为数据库条件更新；Redis 返回错误时无法判断
// 预扣是否已执行，必须失败关闭，避免恢复后旧缓存再次授权同一余额。
func TryReserveUserQuota(id int, quota int) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("user quota cache reserve unavailable: %w", err)
	}
...
func TryReserveTokenQuota(id int, key string, quota int, unlimited bool) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("token quota cache reserve unavailable: %w", err)
	}
	if result == cacheQuotaMiss {
		return reserveTokenQuotaDB(id, quota)
	}
