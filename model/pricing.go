...
type Pricing struct {
	...
	OriginalPrice          *ratio_setting.ModelOriginalPrice `json:"original_price,omitempty"`
...
func updatePricing() {
	...
	// 再补充模型自定义端点：保留渠道原生端点，并追加模型自定义端点。
	...
	for model, groups := range modelGroupsMap {
		...
		if originalPrice, ok := ratio_setting.GetModelOriginalPrice(model); ok {
...
func mergeModelEndpointTypes(defaultEndpoints []string, endpointsConfig string) []string {
	...
	var raw map[string]interface{}
	if err := json.Unmarshal([]byte(endpointsConfig), &raw); err != nil {
		return endpoints
	}
	for k, v := range raw {
		switch v.(type) {
