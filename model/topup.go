...
const (
	...
	PaymentMethodManualWechat = "manual_wechat"
	PaymentMethodManualAlipay = "manual_alipay"
)
...
func AdminCompleteManualTopUp(tradeNo string, overrideAmount int64, callerIp string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		var quotaErr error
		quotaToAdd, quotaErr = common.WalletQuotaFromDecimalStrict(
			decimal.NewFromInt(overrideAmount).Mul(decimal.NewFromFloat(common.QuotaPerUnit)),
		)
		if quotaErr != nil || quotaToAdd <= 0 {
			return ErrInvalidTopUpQuota
		}
